...
export function displayPageNav() {
    if (state.userFound) {
      page.innerHTML = `Repositories: <wbr>${state.firstRepoOnPage} - ${Math.min(state.lastRepoOnPage,state.reposNum)}/${state.reposNum}`;
  ...
  export function displayRepos(start, stop) {
    displayPageNav();
    const html = state.reposList
      .slice(start - 1, stop)
      .map(
        (element) => `<article>
            <header class='result'>
